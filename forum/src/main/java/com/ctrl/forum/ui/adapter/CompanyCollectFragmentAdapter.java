...
public class CompanyCollectFragmentAdapter extends BaseAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (list!=null && list.get(position)!=null){
            ...
            BigDecimal b   =   new   BigDecimal(Double.parseDouble(list.get(position).getDis())/1000.0);
            double   f1   =   b.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();
            holder.tv_distance.setText(f1+"km");

            Arad.imageLoader.load(company.getImg()).placeholder(mcontext.getResources().getDrawable(R.mipmap.image_default)).
                    into(holder.iv_title_photo);
            ...
            if (company.getCouponEnable().equals("0")){
                holder.tv_xianjinquan_name.setVisibility(View.GONE);
                holder.tv_xianjinquan.setVisibility(View.GONE);
            }else{
                holder.tv_xianjinquan_name.setText(company.getCashName());
            }
