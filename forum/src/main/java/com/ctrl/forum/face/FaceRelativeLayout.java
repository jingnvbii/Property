...
public class FaceRelativeLayout extends RelativeLayout implements
	...
	public void onClick(View v) {
		...
		case R.id.tv_photo_album://相册
			if(Arad.preferences.getBoolean("isFromCommentDetail")){
				InvitationCommentDetaioActivity activity=(InvitationCommentDetaioActivity)context;
				activity.goToAlbum();
			}
			else if(Arad.preferences.getBoolean("isFromAPP")) {
				InvitationPinterestDetailActivity activity = (InvitationPinterestDetailActivity) context;
				activity.goToAlbum();
			}else {
				InvitationDetailFromPlatformActivity activity=(InvitationDetailFromPlatformActivity)context;
				activity.goToAlbum();
			}
			break;
		case R.id.tv_photo_graph://拍照
			if(Arad.preferences.getBoolean("isFromCommentDetail")){
				InvitationCommentDetaioActivity activity=(InvitationCommentDetaioActivity)context;
				activity.goToGraph();
			}
			else if(Arad.preferences.getBoolean("isFromAPP")) {
				InvitationPinterestDetailActivity activity2 = (InvitationPinterestDetailActivity) context;
				activity2.goToGraph();
			}else {
				InvitationDetailFromPlatformActivity activity2=(InvitationDetailFromPlatformActivity)context;
				activity2.goToGraph();
			}
			break;
	...
	private void Init_View() {
		...
		ll_add=(LinearLayout)findViewById(R.id.ll_bottom_edit);
