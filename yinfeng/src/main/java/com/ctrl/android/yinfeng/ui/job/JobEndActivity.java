...
public class JobEndActivity extends AppToolBarActivity implements View.OnClickListener {
    ...
    @InjectView(R.id.tv_repairs_pingjia)//评价内容
    TextView tv_repairs_pingjia;
    @InjectView(R.id.tv_repairs_level)//评价等级
    ...
    public void onRequestSuccess(int requestCode) {
        ...
        if(requestCode==1){
            tv_repairs_progress.setText("报修进度：已结束");
            ...
            tv_repairs_pingjia.setText(dao.getRepair().getEvaluateContent());

            if(dao.getRepair().getEvaluateLevel().equals("0")){
                tv_repairs_level.setText("满意");
            } else if(dao.getRepair().getEvaluateLevel().equals("1")){
                tv_repairs_level.setText("不满意");
            }else {
                tv_repairs_level.setVisibility(View.GONE);
            }
