...
import com.ctrl.android.property.eric.ui.adapter.ForumDetailAdapter;
    ...
    public void onClick(View v) {
        if(v == forum_like_btn){
            ...
            if(forumNoteDetail.getIsPraise() == 0){
                forumDao.requestLikeNote(forumNoteDetail.getForumPostId(),AppHolder.getInstance().getMemberInfo().getMemberId());
            } else {
                forumDao.requestCancelLikeNote(forumNoteDetail.getForumPostId(), AppHolder.getInstance().getMemberInfo().getMemberId());
            }
        }
